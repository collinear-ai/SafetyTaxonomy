...
export function parseTaxonomyCSV(csvContent: string): TaxonomyData {
  ...
  for (let i = 2; i < lines.length; i++) {
    ...
    items.push({
      ...
      l2Category,
      l3Category,
      l3Number: 0, // Will be set properly during grouping
      example,
      frameworks,
    })
  }

  // Group items by L1 -> L2 -> L3 hierarchy and assign L3 numbers
  const l1Map = new Map<string, Map<string, TaxonomyItem[]>>()
  const l1CounterMap = new Map<string, number>() // Track counter per L1 category

  items.forEach((item) => {
    if (!l1Map.has(item.l1Category)) {
      l1Map.set(item.l1Category, new Map())
      l1CounterMap.set(item.l1Category, 0) // Initialize counter for this L1 category
    }
    ...
    // Increment the counter for this specific L1 category and assign
    const currentCounter = l1CounterMap.get(item.l1Category)! + 1
    l1CounterMap.set(item.l1Category, currentCounter)

    const itemWithNumber = { ...item, l3Number: currentCounter }
    l2Map.get(item.l2Category)!.push(itemWithNumber)
  })
  ...
  l1Map.forEach((l2Map, l1CategoryName) => {
    ...
    categories.push({
      name: l1CategoryName,
      color:
        CATEGORY_COLORS[l1CategoryName as keyof typeof CATEGORY_COLORS] ||
